$(function(){
  ...
  ws.onmessage = function(message){
    ...
    if (text_width_int >= window_w) {
      ...
      $('#twitter-text-username').animate({opacity: 1},{duration: fadein_speed,easing: 'swing'}).delay((start_delay+1000)+(text_width_int*2));
      ...
      $('#twitter-text-message').animate({marginLeft: text_scroll-10},{duration: text_width_int*2,easing: 'linear'}).delay(display_delay);
      ...
      $("#twitter-text-username").text(message_data[0]);
      $("#twitter-text-message").text(message_data[1]);
      $('#twitter-text-username').animate({opacity: 1},{duration: fadein_speed,easing: 'swing'}).delay(display_delay);
      $('#twitter-text-message').animate({opacity: 1,marginLeft: 0},{duration: fadein_speed,easing: 'swing'}).delay(display_delay);
      $('#twitter-text-username').animate({opacity: 0},{duration: fadeout_speed,easing: 'swing'});
      $('#twitter-text-message').animate({opacity: 0},{duration: fadeout_speed,complete: function(){ws.send('JS'),$('#twitter-text-message').css('margin-left','250px');}});
