  ...
  ws.onmessage = function(message){
    ...
    if (loopcnt == 0){
      ...
      if (text_width_int >= window_w) {
        ...
        $('#twitter-text-message').animate({marginLeft: text_scroll-10},{duration: text_width_int*scrollspeed,easing: 'linear'}).delay(3000);
        ...
        $("#twitter-text-message").text(message.data);
        $('#twitter-text-message').animate({opacity: 1},{queue: false,duration: 2000,easing: 'swing'});
        $('#twitter-text-message').animate({marginLeft: 0},{queue: false,duration: 2000,easing: 'swing'}).delay(5000);
        $('#twitter-text-message').animate({opacity: 0},{duration: 2000,complete: function(){ws.send('JS'),$('#twitter-text-message').css('margin-left','250px');}});
